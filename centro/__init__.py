...
# Asegurar que el sistema encuentre la carpeta centro
...
# Ejecuta la creación automática del esquema al inicio [cite: 305]
Base.metadata.create_all(bind=engine)

st.set_page_config(page_title="Enmilla ERP - Logística", layout="wide")

# Menú lateral para navegación [cite: 433]
    ...
    with st.form("registro_paquete"):
        ...
        if st.form_submit_button("Registrar"):
            if t_number and sender and recipient and address:
                ...
                if exists:
                    st.error("Error: El número de seguimiento ya existe.") [cite: 435]
                else:
                    new_pkg = Package(
                    ...
                    db.add(new_pkg)
                    db.commit()
                    st.success(f"Paquete {t_number} registrado con éxito.") [cite: 435]
            else:
...
# --- MÓDULO DE SEGUIMIENTO --- [cite: 376]
    ...
    if search_id:
        ...
        if pkg:
            ...
            st.subheader("Historial de Movimientos") [cite: 382]
...
# --- MÓDULO DE DESPACHO --- [cite: 385]
    ...
    if track_num:
        ...
        if pkg:
            ...
            if st.button("Marcar como Entregado y Generar POD"): [cite: 392]
                pkg.status = "Entregado"
                pkg.is_delivered = True
                db.commit()
                st.success("Paquete entregado. Comprobante (POD) generado.") [cite: 401]
        else:
            st.error("Paquete no encontrado.")
